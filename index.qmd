...
### Figure 1: Post-Surgical Outcomes by Patient {#fig-plot-outcomes}
...
# Load the data (ensure it's available)
patients_df = pd.read_csv('patients_data.csv')
...
# Plot Doc Duck (triangles, coral)
ax.scatter(duck_data['patient'], duck_data['post_surgical_score'], 
           marker='^', color='#E15759', s=60, alpha=0.7, 
           label='Doc Duck', zorder=3)

# Add horizontal mean lines (dashed)
ax.axhline(y=dreamy_mean, color='#4E79A7', linestyle='--', linewidth=2, alpha=0.8, zorder=2)
ax.axhline(y=duck_mean, color='#E15759', linestyle='--', linewidth=2, alpha=0.8, zorder=2)

# Add mean annotations with shape symbols
ax.text(len(patients_df) * 0.98, dreamy_mean, f'○ {dreamy_mean:.2f}', 
        ha='right', va='bottom', fontsize=10, color='#4E79A7', 
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8), zorder=4)
ax.text(len(patients_df) * 0.98, duck_mean, f'△ {duck_mean:.2f}', 
        ha='right', va='top', fontsize=10, color='#E15759', 
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8), zorder=4)
...
### Figure 5: Post-Surgical Outcomes by Patient (Randomized Assignment) {#fig-plot-outcomes-randomized}
...
# Add mean annotations with shape symbols
ax.text(len(patients_randomized_df) * 0.98, dreamy_mean, f'○ {dreamy_mean:.2f}', 
        ha='right', va='bottom', fontsize=10, color='#4E79A7', 
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8), zorder=4)
ax.text(len(patients_randomized_df) * 0.98, duck_mean, f'△ {duck_mean:.2f}', 
        ha='right', va='top', fontsize=10, color='#E15759', 
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8), zorder=4)
...
# Add mean annotations with shape symbols
# Position annotations near the right side of the plot
...
x_padding = (patients_df['severity'].max() - patients_df['severity'].min()) * 0.1
y_padding = (patients_df['post_surgical_score'].max() - patients_df['post_surgical_score'].min()) * 0.1
ax.set_xlim(patients_df['severity'].min() - x_padding, patients_df['severity'].max() + x_padding)
ax.set_ylim(patients_df['post_surgical_score'].min() - y_padding, patients_df['post_surgical_score'].max() + y_padding)
